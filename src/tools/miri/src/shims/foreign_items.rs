...
trait EvalContextExtPriv<'tcx>: crate::MiriInterpCxExt<'tcx> {
    ...
    fn emulate_foreign_item_inner(
        ...
        // ```
        // "shim_name" => {
        //     let [arg1, arg2, arg3] = this.check_shim(fnabi, ExternAbi::C { unwind: false }, link_name, args)?;
        ...
        match link_name.as_str() {
            ...
            "miri_start_unwind" => {
                let [payload] = this.check_shim(fnabi, ExternAbi::Rust, link_name, args)?;
                this.handle_miri_start_unwind(payload)?;
                return interp_ok(EmulateItemResult::NeedsUnwind);
            }
            "miri_run_provenance_gc" => {
                let [] = this.check_shim(fnabi, ExternAbi::Rust, link_name, args)?;
                this.run_provenance_gc();
            }
            "miri_get_alloc_id" => {
                let [ptr] = this.check_shim(fnabi, ExternAbi::Rust, link_name, args)?;
            ...
            "miri_print_borrow_state" => {
                let [id, show_unnamed] = this.check_shim(fnabi,  ExternAbi::Rust, link_name, args)?;
            ...
            "miri_backtrace_size" => {
                this.handle_miri_backtrace_size(fnabi,link_name, args, dest)?;
            }
            // Obtains a Miri backtrace. See the README for details.
            "miri_get_backtrace" => {
                // `check_shim` happens inside `handle_miri_get_backtrace`.
                this.handle_miri_get_backtrace(fnabi,link_name, args, dest)?;
            }
            // Resolves a Miri backtrace frame. See the README for details.
            "miri_resolve_frame" => {
                // `check_shim` happens inside `handle_miri_resolve_frame`.
                this.handle_miri_resolve_frame(fnabi,link_name, args, dest)?;
            }
            ...
            "miri_resolve_frame_names" => {
                this.handle_miri_resolve_frame_names(fnabi,link_name, args)?;
            }
            // Writes some bytes to the interpreter's stdout/stderr. See the
            // README for details.
            "miri_write_to_stdout" | "miri_write_to_stderr" => {
                let [msg] = this.check_shim(fnabi, ExternAbi::Rust, link_name, args)?;
            ...
            "miri_promise_symbolic_alignment" => {
                ...
                let [ptr, align] = this.check_shim(fnabi, ExternAbi::Rust, link_name, args)?;
                let ptr = this.read_pointer(ptr)?;
            ...
            "exit" => {
                let [code] =
                    this.check_shim(fnabi, ExternAbi::C { unwind: false }, link_name, args)?;
                let code = this.read_scalar(code)?.to_i32()?;
                throw_machine_stop!(TerminationInfo::Exit { code: code.into(), leak_check: false });
            }
            "abort" => {
                let [] = this.check_shim(fnabi, ExternAbi::C { unwind: false }, link_name, args)?;
            ...
            "__rust_alloc" | "miri_alloc" => {
                let default = |this: &mut MiriInterpCx<'tcx>| {
                    ...
                    let [size, align] = this.check_shim(fnabi, ExternAbi::Rust, link_name, args)?;
                    let size = this.read_target_usize(size)?;
                    let align = this.read_target_usize(align)?;
            ...
            | "tgammaf"
            => {
                let [f] = this.check_shim(fnabi, ExternAbi::C { unwind: false }, link_name, args)?;
            ...
            | "fdimf"
            => {
                let [f1, f2] = this.check_shim(fnabi, ExternAbi::C { unwind: false }, link_name, args)?;
            ...
            | "scalbn"
            => {
                let [x, exp] = this.check_shim(fnabi, ExternAbi::C { unwind: false }, link_name, args)?;
            ...
            name if name.starts_with("llvm.ctpop.v") => {
                let [op] = this.check_shim(fnabi, ExternAbi::C { unwind: false }, link_name, args)?;
            ...
            "llvm.aarch64.isb" if this.tcx.sess.target.arch == "aarch64" => {
                let [arg] = this.check_shim(fnabi, ExternAbi::Unadjusted, link_name, args)?;
                ...
                return match this.tcx.sess.target.os.as_ref() {
                    ...
                    "wasi" =>
                        shims::wasi::foreign_items::EvalContextExt::emulate_foreign_item_inner(
                            this, link_name, fnabi,args, dest,
                        ),
                    "windows" =>
