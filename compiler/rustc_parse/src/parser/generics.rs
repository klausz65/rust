        ...
        loop {
            ...
            let predicate =
                self.collect_tokens(None, attrs, ForceCollect::Yes, |this, attrs| {
                    for attr in &attrs {
                        self.psess.gated_spans.gate(sym::cfg_attribute_in_where, attr.span);
                    }
                    let kind =
                        if this.check_lifetime() && this.look_ahead(1, |t| !t.is_like_plus()) {
                            let lifetime = this.expect_lifetime();
                            // Bounds starting with a colon are mandatory, but possibly empty.
                            this.expect(&token::Colon)?;
                            let bounds = this.parse_lt_param_bounds();
                            Some(ast::WherePredicateKind::RegionPredicate(
                                ast::WhereRegionPredicate { lifetime, bounds },
                            ))
